...
<!-- prettier-ignore -->
```ts
const response = await client.alive.check().catch(async (err) => {
  if (err instanceof Compeer.APIError) {
...
const response = await client.alive.check().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: response, response: raw } = await client.alive.check().withResponse();
...
1. Via the `COMPEER_LOG` environment variable
...
const client = new Compeer({
  logger: logger.child({ name: 'Compeer' }),
